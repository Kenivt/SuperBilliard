...
namespace GFLearn.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        private const string CSharpCodePath = "Assets/GameMain/Scripts/DataTables";
        private const string CSharpCodeTemplateFileName = "Assets/GameMain/Configs/DataTableCodeTemplate.txt";
        private const string NameSpace = "SuperBilliard";
        ...
        {
            return new DataTableProcessor(dataTablePath, Encoding.GetEncoding("GB2312"), 1, 2, null, 3, 4, 1);
        }
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    ...
                    else if (dataTableProcessor.GetLanguageKeyword(i) == "int[]")
                    {
                        stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i), "ArrayInt").AppendLine();
                    }
                    else
                    {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.Read{1}();", dataTableProcessor.GetName(i), "ArrayInt").AppendLine();
                    }
                    else
                    {
