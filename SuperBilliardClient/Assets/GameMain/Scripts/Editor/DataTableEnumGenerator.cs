...
using UnityEditor;
using GameFramework;
using GFLearn.Editor.DataTableTools;
...
namespace Flower
{
    public static class DataTableEnumGenerator
    {
        ...
        private static void GenerateDataTableEnum()
        {
            ...
            {
                string path = Utility.Path.GetRegularPath(Path.Combine(dataTablePath, dataTableName + ".txt"));
                DataTableProcessor dataTableProcessor = DataTableGenerator.CreateDataTableProcessor(path, dataTableName);
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "SuperBilliard");
