...
{
    public class FancyOnlineBattleBuilder : IBattleBuilder
    {
        ...
        IEnumerator<IMyCoroutineItem> LoadGame(BattleDataItem battleItem)
        {
            ...
            for (int i = 0; i < battleItem.BilliardIdArray.Length; i++)
            {
                ...
                GameEntry.Entity.ShowBilliard<FancyBilliardLogic>(EnumEntity.FancyBilliard, BilliardData.Create(billiardDataItem, serilizeId));
            ...
            //加载UI
            int? uiId = GameEntry.UI.OpenUIForm(EnumUIForm.BilliardMessage);
            if (uiId.HasValue)
            {
